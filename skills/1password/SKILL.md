...
1. **Open 1Password app** → Developer → Environments → Create Environment
2. **Import your .env** → Select the file or paste variables manually
3. **Set Mount Destination** → Destinations tab → Add path (e.g., `.env.local`)
4. **Activate** → Environment mounts automatically when 1Password is running
